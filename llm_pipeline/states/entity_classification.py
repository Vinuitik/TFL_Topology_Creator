...
map to existing properties, and generate rdfs:label and rdfs:comment for each via the lightweight entity model.
Runs after extraction/entity_linking.
"""
...
from utils import cosine, find_knn_by_embedding
...
def _is_literal(value: str) -> bool:
    """Robust literal detection using regex (from version 2)."""
...
def _run_relation_batch(
    ...
    # Combine the allowed properties list with V1's advanced contextual querying
    allowed_str = ", ".join(f'"{a}"' for a in allowed_props)
    lines = [f"Allowed List: [{allowed_str}]\n"]
...
def run_entity_classification(state: PipelineState) -> PipelineState:
    ...
    # Allow mapping to existing RAG catalog properties (from Version 2)
    ...
    # Step 2: classify new relations (context + embedding KNN + map to allowed + deterministic literal override)
    ...
    for batch_idx, i in enumerate(range(0, len(relations_to_classify), _BATCH), 1):
        ...
        results = _run_relation_batch(
            batch, relation_context, has_literal, labeled_relations, relation_embeddings, allowed_props
        )
        for name, r in zip(batch, results):
            mapped_prop = r.get("property", name)  # Use mapped property if identified
    ...
    for batch_idx, i in enumerate(range(0, len(items_to_annotate), _BATCH), 1):
        ...
        for item, r in zip(batch, results):
            name = item["name"]
            # Fall back to mapped_prop label rather than `name` if missing
            catalog[name]["label"] = r.get("label", catalog[name]["label"]) or catalog[name]["label"]
